...
(async () => {
  ...
  sections.forEach((section, index) => {
    ...
    const list = item.querySelector('.section-video-list'); current.forEach(video => { const link = document.createElement('button'); link.type = 'button'; const watched = watchedVideos.has(videoKey(video)); link.className = `section-video-link w-full ${watched ? 'is-watched' : ''}`; const thumbnail = video.thumbnailUrl || video.thumbnail_url || ''; const duration = formatDuration(video.duration); link.innerHTML = `${thumbnail ? `<img class="section-video-thumb" src="${escapeHtml(thumbnail)}" alt="" onerror="this.style.display='none'" />` : '<span class="section-video-thumb flex items-center justify-center text-orange">▶</span>'}<span class="min-w-0 flex flex-col gap-0.5"><span class="truncate">${escapeHtml(video.title || 'Vídeo educativo')}</span>${duration ? `<small class="text-[10px] text-muted">${escapeHtml(duration)}</small>` : ''}</span><span class="video-watched-toggle ${watched ? 'is-checked' : ''}" role="checkbox" aria-checked="${watched}" tabindex="0" title="Marcar como ${watched ? 'não assistido' : 'assistido'}">✓</span>`; link.title = video.title || ''; const toggleWatched = event => { event.preventDefault(); event.stopPropagation(); const next = !watchedVideos.has(videoKey(video)); setWatched(video, next); link.classList.toggle('is-watched', next); const marker = link.querySelector('.video-watched-toggle'); marker.classList.toggle('is-checked', next); marker.setAttribute('aria-checked', String(next)); marker.title = `Marcar como ${next ? 'não assistido' : 'assistido'}`; }; link.addEventListener('click', event => { if (event.target.closest('.video-watched-toggle')) return; setWatched(video); render(section.name); }); link.querySelector('.video-watched-toggle').addEventListener('click', toggleWatched); link.querySelector('.video-watched-toggle').addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') toggleWatched(event); }); list.appendChild(link); });
