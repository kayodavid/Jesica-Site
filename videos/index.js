...
(async () => {
  ...
  function formatDuration(value) { if (value === null || value === undefined || value === '') return 'Duração não informada'; if (typeof value === 'number' || /^\d+(\.\d+)?$/.test(String(value))) { const seconds = Math.max(0, Math.round(Number(value))); const minutes = Math.floor(seconds / 60); const rest = String(seconds % 60).padStart(2, '0'); return `${minutes}:${rest}`; } return String(value); }
  ...
  sections.forEach((section, index) => {
    ...
    const list = item.querySelector('.section-video-list'); current.forEach(video => { const link = document.createElement('button'); link.type = 'button'; link.className = 'section-video-link w-full'; const thumbnail = video.thumbnailUrl || video.thumbnail_url || ''; link.innerHTML = `${thumbnail ? `<img class="section-video-thumb" src="${escapeHtml(thumbnail)}" alt="" onerror="this.style.display='none' />` : '<span class="section-video-thumb flex items-center justify-center text-orange">▶</span>'}<span class="min-w-0 flex flex-col gap-0.5"><span class="truncate">${escapeHtml(video.title || 'Vídeo educativo')}</span><small class="text-[10px] text-muted">${escapeHtml(formatDuration(video.duration))}</small></span>`; link.title = video.title || ''; link.addEventListener('click', () => render(section.name)); list.appendChild(link); });
