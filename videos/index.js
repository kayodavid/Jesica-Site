...
(async () => {
  ...
  (Array.isArray(rawSections) ? rawSections : []).forEach(s => {
    const name = (s.name || s || '').trim();
    if (name) canonicalSectionNames.set(name.toLowerCase(), name);
  });
  ['GERAL', 'PERDA DE PESO', 'HIPERTROFIA'].forEach(name => {
    if (!canonicalSectionNames.has(name.toLowerCase())) {
      canonicalSectionNames.set(name.toLowerCase(), name);
    }
  });

  function getCanonicalTheme(theme) {
    const trimmed = String(theme || 'GERAL').trim() || 'GERAL';
    const lower = trimmed.toLowerCase();
    if (canonicalSectionNames.has(lower)) {
      return canonicalSectionNames.get(lower);
    }
    canonicalSectionNames.set(lower, trimmed);
    return trimmed;
  }
  ...
  videos.forEach(video => {
    const name = getCanonicalTheme(video.theme);
    if (!name.startsWith('__') && !name.startsWith('Calculadoras')) {
  ...
  (Array.isArray(rawSections) ? rawSections : []).forEach(section => {
    const name = getCanonicalTheme(section.name || section);
    if (!sectionMap.has(name)) {
      sectionMap.set(name, typeof section === 'object' ? { ...section, name } : { name, cover_image: '' });
    }
  });

  Object.keys(groups).forEach(name => {
    if (!sectionMap.has(name)) {
      sectionMap.set(name, { name, cover_image: '' });
    }
  });

  const sections = [...sectionMap.values()]
    .filter(section => (groups[section.name] || []).length > 0)
    .sort((a, b) => settings.video_section_order === 'alpha'
      ? byName(a, b)
      : byRecent(
          { createdAt: Math.max(...(groups[a.name] || []).map(v => new Date(v.createdAt || v.created_at || 0).getTime()), 0) },
          { createdAt: Math.max(...(groups[b.name] || []).map(v => new Date(v.createdAt || v.created_at || 0).getTime()), 0) }
        )
    );
  ...
  function protectedEmbedUrl(video) {
    const raw = video.embedUrl || video.embed_url || '';
    try {
  ...
  function configurePlayerProtection(playerUrl) {
    try {
  ...
  function sectionCover(section) {
    return section.cover_image || groups[section.name]?.[0]?.thumbnailUrl || groups[section.name]?.[0]?.thumbnail_url || '';
  }
  ...
  function renderLanding() {
    ...
    document.getElementById('no-videos').classList.add('hidden');
  ...
  function render(name) {
    ...
    const section = sections.find(item => item.name.toLowerCase() === String(name).toLowerCase()) || sections[0];
    if (!section) {
    ...
    current.forEach(video => {
      ...
      card.addEventListener('click', () => {
        ...
        const playerUrl = protectedEmbedUrl(video);
        if (!playerUrl) return;
        configurePlayerProtection(playerUrl);
        frame.src = playerUrl;
    ...
    nav.querySelectorAll('.section-nav-card').forEach(button => button.classList.toggle('active', button.dataset.section.toLowerCase() === section.name.toLowerCase()));
  ...
  sections.forEach((section, index) => {
    ...
    current.forEach(video => {
      ...
      link.innerHTML = `${thumbnail ? `<img class="section-video-thumb" src="${escapeHtml(thumbnail)}" alt="" onerror="this.style.display='none'" />` : '<span class="section-video-thumb flex items-center justify-center text-orange">▶</span>'}<span class="min-w-0 flex flex-col gap-0.5"><span class="truncate">${escapeHtml(video.title || 'Vídeo educativo')}</span>${duration ? `<small class="text-[10px] text-muted">${escapeHtml(duration)}</small>` : ''}</span>`;
    ...
    item.querySelector('.section-expand').addEventListener('click', event => {
  ...
  if (sections.length) {
    ...
    landing.classList.remove('hidden');
    landing.innerHTML = '<div class="col-span-full border border-white/10 bg-panel rounded-2xl p-10 text-center text-muted">Nenhum conteúdo publicado ainda.</div>';
    libraryHero.classList.add('hidden');
  }
})();
