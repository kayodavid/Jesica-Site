...
(async () => {
  ...
  const watchedStorageKey = `jessicamelo_watched_videos_${session.email || session.user_id || session.id || 'patient'}`;
  const watchedVideos = new Set(JSON.parse(localStorage.getItem(watchedStorageKey) || '[]'));
  function videoKey(video) { return String(video.id || video.title || video.embedUrl || video.embed_url || ''); }
  function saveWatchedVideos() { localStorage.setItem(watchedStorageKey, JSON.stringify([...watchedVideos])); }
  function setWatched(video, value = true) { const key = videoKey(video); if (!key) return; value ? watchedVideos.add(key) : watchedVideos.delete(key); saveWatchedVideos(); }

  function formatDuration(value) { if (value === null || value === undefined || value === '') return ''; if (typeof value === 'number' || /^\d+(\.\d+)?$/.test(String(value))) { const seconds = Math.max(0, Math.round(Number(value))); const minutes = Math.floor(seconds / 60); const rest = String(seconds % 60).padStart(2, '0'); return `${minutes}:${rest}`; } return String(value); }
  function protectedEmbedUrl(video) { const raw = video.embedUrl || video.embed_url || ''; try { const url = new URL(raw); if (url.hostname.includes('vimeo.com')) { url.searchParams.set('dnt', '1'); url.searchParams.set('title', '0'); url.searchParams.set('byline', '0'); url.searchParams.set('portrait', '0'); } return url.toString(); } catch { return ''; } }
  function configurePlayerProtection(playerUrl) { try { const host = new URL(playerUrl).hostname; const isYouTube = host.includes('youtube.com') || host === 'youtu.be'; if (isYouTube) { frame.removeAttribute('sandbox'); frame.removeAttribute('referrerpolicy'); } else { frame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-presentation'); frame.setAttribute('referrerpolicy', 'no-referrer'); } } catch {} }
  ...
  function render(name) {
    ...
    current.forEach(video => {
      ...
      const watched = watchedVideos.has(videoKey(video));
      card.innerHTML = `<div class="relative aspect-video bg-[#30323a] overflow-hidden">${thumbnail ? `<img src="${escapeHtml(thumbnail)}" alt="${escapeHtml(video.title)}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.style.display='none'" />` : '<div class="w-full h-full flex items-center justify-center text-orange text-5xl">▶</div>'}<div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div><span class="absolute inset-0 flex items-center justify-center"><span class="w-14 h-14 rounded-full bg-orange text-dark flex items-center justify-center text-xl shadow-lg group-hover:scale-110 transition-transform">▶</span></span></div><div class="video-copy relative p-4 min-h-[126px] pr-12"><h2 class="font-bold text-sm sm:text-base leading-snug line-clamp-2">${escapeHtml(video.title)}</h2><p class="text-xs text-orange mt-3 italic">${escapeHtml(section.name)}</p><p class="text-xs text-gray-400 mt-2 line-clamp-2">${escapeHtml(video.description || 'Conteúdo educativo para apoiar sua jornada.')}</p><button type="button" class="video-status-toggle ${watched ? 'is-watched' : ''}" aria-label="${watched ? 'Marcar como não visto' : 'Marcar como visto'}" title="${watched ? 'Visto — clique para marcar como não visto' : 'Não visto — clique para marcar como visto'}">${watched ? '✓' : '<span class="video-status-dot"></span>'}</button></div>`;
      const statusToggle = card.querySelector('.video-status-toggle');
      const toggleCardStatus = event => { event.preventDefault(); event.stopPropagation(); const next = !watchedVideos.has(videoKey(video)); setWatched(video, next); statusToggle.classList.toggle('is-watched', next); statusToggle.setAttribute('aria-label', next ? 'Marcar como não visto' : 'Marcar como visto'); statusToggle.title = next ? 'Visto — clique para marcar como não visto' : 'Não visto — clique para marcar como visto'; };
      statusToggle.addEventListener('click', toggleCardStatus);
      card.addEventListener('click', event => { if (event.target.closest('.video-status-toggle')) return; setWatched(video); statusToggle.classList.add('is-watched'); statusToggle.setAttribute('aria-label', 'Marcar como não visto'); statusToggle.title = 'Visto — clique para marcar como não visto'; document.getElementById('modal-theme').textContent = section.name; document.getElementById('modal-title').textContent = video.title; const playerUrl = protectedEmbedUrl(video); if (!playerUrl) return; configurePlayerProtection(playerUrl); frame.src = playerUrl; modal.classList.remove('hidden'); modal.classList.add('flex'); });
  ...
  sections.forEach((section, index) => {
    ...
    const list = item.querySelector('.section-video-list'); current.forEach(video => { const link = document.createElement('button'); link.type = 'button'; link.className = 'section-video-link w-full'; const thumbnail = video.thumbnailUrl || video.thumbnail_url || ''; const duration = formatDuration(video.duration); link.innerHTML = `${thumbnail ? `<img class="section-video-thumb" src="${escapeHtml(thumbnail)}" alt="" onerror="this.style.display='none'" />` : '<span class="section-video-thumb flex items-center justify-center text-orange">▶</span>'}<span class="min-w-0 flex flex-col gap-0.5"><span class="truncate">${escapeHtml(video.title || 'Vídeo educativo')}</span>${duration ? `<small class="text-[10px] text-muted">${escapeHtml(duration)}</small>` : ''}</span>`; link.title = video.title || ''; link.addEventListener('click', () => render(section.name)); list.appendChild(link); });
