...
(async () => {
  ...
  function render(name) {
    ...
    current.forEach(video => {
      ...
      card.innerHTML = `<div class="relative aspect-video bg-[#30323a] overflow-hidden">${thumbnail ? `<img src="${escapeHtml(thumbnail)}" alt="${escapeHtml(video.title)}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.style.display='none'" />` : '<div class="w-full h-full flex items-center justify-center text-orange text-5xl">▶</div>'}<div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div><span class="absolute inset-0 flex items-center justify-center"><span class="w-14 h-14 rounded-full bg-orange text-dark flex items-center justify-center text-xl shadow-lg group-hover:scale-110 transition-transform">▶</span></span></div><div class="video-copy p-4 min-h-[126px]"><div class="flex items-start justify-between gap-2"><h2 class="font-bold text-sm sm:text-base leading-snug line-clamp-2">${escapeHtml(video.title)}</h2><span class="video-status-tag ${watched ? 'is-watched' : ''}">${watched ? 'Visto' : 'Não visto'}</span></div><p class="text-xs text-orange mt-3 italic">${escapeHtml(section.name)}</p><p class="text-xs text-gray-400 mt-2 line-clamp-2">${escapeHtml(video.description || 'Conteúdo educativo para apoiar sua jornada.')}</p></div>`;
      card.addEventListener('click', () => { setWatched(video); const statusTag = card.querySelector('.video-status-tag'); statusTag.textContent = 'Visto'; statusTag.classList.add('is-watched'); document.getElementById('modal-theme').textContent = section.name; document.getElementById('modal-title').textContent = video.title; const playerUrl = protectedEmbedUrl(video); if (!playerUrl) return; configurePlayerProtection(playerUrl); frame.src = playerUrl; modal.classList.remove('hidden'); modal.classList.add('flex'); });
