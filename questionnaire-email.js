...
function buildReminderTestEmail({ reminder: rawReminder, patientName = '' }) {
  ...
  const htmlContent = `<!doctype html><html lang="pt-BR"><head><meta name="color-scheme" content="light"></head><body style="margin:0;background:#faf8f3;font-family:Arial,Helvetica,sans-serif;color:#3d3226;line-height:1.6"><div style="max-width:600px;margin:0 auto;padding:32px 18px"><div style="overflow:hidden;border:1px solid rgba(168,139,54,.22);border-radius:18px;background:#fff;box-shadow:0 8px 24px rgba(61,50,38,.1)"><div style="background:#a88b36;color:#fff;padding:25px 28px"><p style="margin:0 0 5px;font-size:11px;letter-spacing:.12em;text-transform:uppercase;opacity:.9">${brand}</p><p style="margin:0 0 8px;font-size:10px;letter-spacing:.1em;text-transform:uppercase;opacity:.82">E-mail de teste</p><h1 style="margin:0;font-size:24px;line-height:1.25">${title}</h1></div><div style="padding:28px"><div style="margin:0 0 22px;border:1px solid #ead9a6;border-radius:12px;background:#fffaf0;padding:13px 15px;color:#725b20;font-size:13px;line-height:1.5"><strong>Mensagem de teste:</strong> este e-mail foi enviado somente para validar o lembrete. O botão abaixo é apenas ilustrativo e não representa um convite real.</div><p style="margin:0 0 16px;font-size:16px;font-weight:700;color:#3d3226">${safeSubject}</p><p style="margin:0 0 20px;color:#3d3226;line-height:1.7">${safeMessage}</p><p style="margin:0;color:#6d6255;font-size:12px;line-height:1.6">Os dados exibidos são exemplos e o botão abaixo não representa um convite real.</p><p style="margin:20px 0 0"><a href="${safeRecipient}" style="display:inline-block;background:#a88b36;border-radius:10px;color:#fff;padding:12px 20px;text-decoration:none;font-weight:700">Responder questionário</a></p></div></div><p style="font-size:12px;color:#827766;text-align:center;margin:18px 0 0">Jessica Melo Nutricionista · Teste de lembrete</p></div></body></html>`;
  return { subject:`Teste de lembrete — ${subject}`.slice(0, 180), htmlContent };
}

const DEFAULT_SERVER_REMINDERS = [
  { id:'new_quiz', title:'Novo Questionário', active:true, email:true, normalHoursOnly:true, subject:'Seu questionário está pronto', message:'Olá, {primeiro_nome}!\n\nSeu questionário {questionario} já está disponível. Reserve alguns minutos para responder clicando no botão abaixo.', routine:{ triggers:[{ offset:0, unit:'minutes', relation:'after', time:'' }] } },
  { id:'service_ending', title:'Serviço Finalizando', active:true, email:true, normalHoursOnly:true, subject:'Seu acompanhamento está chegando ao fim', message:'Olá, {primeiro_nome}!\n\nSeu acompanhamento está próximo do fim. Faltam {dias_restantes} dias para o encerramento. Se precisar de ajuda, fale comigo.', routine:{ triggers:[{ offset:3, unit:'days', relation:'before', time:'' }, { offset:8, unit:'hours', relation:'before', time:'' }] } },
  { id:'response_due', title:'Prazo de Resposta', active:true, email:true, normalHoursOnly:true, subject:'Lembrete: prazo do questionário', message:'Olá, {primeiro_nome}!\n\nO prazo para responder {questionario} termina em {prazo}. Reserve alguns minutos e envie suas respostas clicando no botão abaixo.', routine:{ triggers:[{ offset:12, unit:'hours', relation:'before', time:'' }] } }
];
...
function normalizeServerReminder(item, fallback) { const data = item && typeof item === 'object' ? item : {}; const routine = data.routine && typeof data.routine === 'object' ? data.routine : {}; const sourceTriggers = Array.isArray(routine.triggers) && routine.triggers.length ? routine.triggers : fallback.routine.triggers; return { ...fallback, ...data, id:fallback.id, title:usableText(data.title) || fallback.title, active:data.active !== false, email:data.email !== false, normalHoursOnly:data.normalHoursOnly !== false, subject:usableText(data.subject) || fallback.subject, message:usableText(data.message) || fallback.message, routine:{ ...fallback.routine, ...routine, triggers:(sourceTriggers || []).slice(0, 8).map(normalizeServerReminderTrigger) } }; }
...
function reminderTriggerAt(baseAt, trigger, normalHoursOnly = true) { const baseTimestamp = Date.parse(baseAt); if (!Number.isFinite(baseTimestamp)) return ''; const direction = trigger?.relation === 'after' ? 1 : -1; const targetTimestamp = baseTimestamp + direction * reminderOffsetMilliseconds(trigger); let targetIso; if (trigger?.time) targetIso = localDateTimeToIso(localDateFromTimestamp(targetTimestamp), trigger.time); else targetIso = new Date(targetTimestamp).toISOString(); if (!normalHoursOnly) return targetIso; return clampToNormalHours(targetIso); }
...
function buildReminderJobs({ reminders, records, invitation, quiz, accessToken, scheduledAt, expiresAt, parentScheduleKey = '' }) {
  ...
  if (responseReminder?.active && responseReminder.email) {
    const normalHours = responseReminder.normalHoursOnly !== false;
  ...
  if (serviceReminder?.active && serviceReminder.email) {
    const normalHours = serviceReminder.normalHoursOnly !== false;
