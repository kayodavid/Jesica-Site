...
async function getReminderTemplate(sessionToken) {
  try { const records = await listStoredQuestionnaireRecords(sessionToken); const record = records.find(item => recordTheme(item) === REMINDER_TEMPLATE_THEME || recordSource(item) === REMINDER_TEMPLATE_SOURCE); if (!record) return getEmailTemplate(sessionToken); let data = {}; try { data = JSON.parse(record.description || '{}'); } catch {} return normalizeEmailTemplate(data); } catch (error) { console.error('Reminder template load error:', error.message); try { return getEmailTemplate(sessionToken); } catch { return defaultEmailTemplate(); } }
}
...
const DEFAULT_SERVER_REMINDERS = [
  ...
  { id:'response_due', title:'Prazo de Resposta', active:true, email:true, normalHoursOnly:true, subject:'Lembrete: prazo do questionário', message:'Olá, {primeiro_nome}!\n\nO prazo para responder {questionario} termina em {prazo}. Reserve alguns minutos e envie suas respostas pelo botão abaixo.', routine:{ triggers:[{ offset:12, unit:'hours', relation:'before', time:'' }] } }
];
