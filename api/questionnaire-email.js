...
function formatReminderDeadline(value) { const timestamp = Date.parse(value); if (!Number.isFinite(timestamp)) return ''; const dateStr = new Intl.DateTimeFormat('pt-BR', { dateStyle:'long', timeZone:'America/Sao_Paulo' }).format(new Date(timestamp)); const timeStr = new Intl.DateTimeFormat('pt-BR', { hour:'2-digit', minute:'2-digit', hour12:false, timeZone:'America/Sao_Paulo' }).format(new Date(timestamp)); return `${dateStr} às ${timeStr}`; }
