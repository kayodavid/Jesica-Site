...
function normalizePatientCalculator(calculator) { if (!calculator) return calculator; return { ...calculator, calculatorType: calculator.calculatorType || calculator.calculator_type || 'bmi', title: calculator.title || 'Calculadora', description: calculator.description || '', published: calculator.published !== false, createdAt: calculator.createdAt || calculator.created_at, updatedAt: calculator.updatedAt || calculator.updated_at }; }
...
export const db = {
  ...
  async getPatientCalculators() { try { return (await rpc('app_list_patient_calculators', { p_token: token() }) || []).map(normalizePatientCalculator); } catch { return JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').map(normalizePatientCalculator); } },
  ...
  async addPatientCalculator(calculatorType, title, description, published = true) { try { return await rpc('app_add_patient_calculator', { p_token: token(), p_calculator_type: calculatorType, p_title: title.trim(), p_description: (description || '').trim(), p_published: published }); } catch { const calculators = JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]'); const calculator = { id: crypto.randomUUID(), calculator_type: calculatorType, title: title.trim(), description: (description || '').trim(), published, created_at: new Date().toISOString() }; calculators.unshift(calculator); localStorage.setItem(DB_KEY_CALCULATORS, JSON.stringify(calculators)); return { success: true, id: calculator.id }; } },
  async updatePatientCalculator(id, calculatorType, title, description, published) { try { return await rpc('app_update_patient_calculator', { p_token: token(), p_id: id, p_calculator_type: calculatorType, p_title: title.trim(), p_description: (description || '').trim(), p_published: published }); } catch { const calculators = JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').map(calculator => calculator.id === id ? { ...calculator, calculator_type: calculatorType, title: title.trim(), description: (description || '').trim(), published, updated_at: new Date().toISOString() } : calculator); localStorage.setItem(DB_KEY_CALCULATORS, JSON.stringify(calculators)); return { success: true }; } },
  async deletePatientCalculator(id) { try { return await rpc('app_delete_patient_calculator', { p_token: token(), p_id: id }); } catch { const calculators = JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').filter(calculator => calculator.id !== id); localStorage.setItem(DB_KEY_CALCULATORS, JSON.stringify(calculators)); return { success: true }; } },
