...
export const db = {
  ...
  async addBlog(title, excerpt, content, published = true) { try { return await rpc('app_add_blog', { p_token: token(), p_title: title.trim(), p_excerpt: (excerpt || '').trim(), p_content: content, p_published: published }); } catch { return { success: false, message: 'Não foi possível publicar o blog no banco.' }; } },
  async updateBlog(id, title, excerpt, content, published = true) { try { return await rpc('app_update_blog', { p_token: token(), p_id: id, p_title: title.trim(), p_excerpt: (excerpt || '').trim(), p_content: content, p_published: published }); } catch { return { success: false, message: 'Não foi possível atualizar o blog.' }; } },
