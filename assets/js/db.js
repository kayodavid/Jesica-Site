...
export const db = {
  ...
  async getPatientCalculators() { const patientFallback = () => currentSession()?.role === 'patient' ? [normalizePatientCalculator(DEFAULT_PATIENT_CALCULATOR)] : []; try { const calculators = (await rpc('app_list_videos', { p_token: token() }) || []).filter(video => video.theme === CALCULATOR_THEME || video.theme === CALCULATOR_HIDDEN_THEME).map(normalizePatientCalculator); return calculators.length ? calculators : patientFallback(); } catch { const local = JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').map(normalizePatientCalculator); return local.length ? local : patientFallback(); } },
