...
export const db = {
  ...
  async getPatientCalculators() { const local = JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').map(normalizePatientCalculator); let remote = []; try { remote = (await rpc('app_list_videos', { p_token: token() }) || []).filter(isCalculatorRecord).map(normalizePatientCalculator); } catch {} const merged = [...remote]; local.forEach(calculator => { if (!merged.some(item => item.id === calculator.id)) merged.push(calculator); }); const complete = [...merged]; DEFAULT_PATIENT_CALCULATORS.map(normalizePatientCalculator).forEach(defaultCalculator => { if (!complete.some(item => item.calculatorType === defaultCalculator.calculatorType)) complete.push(defaultCalculator); }); return complete; },
  async getPublishedPatientCalculators() { return (await this.getPatientCalculators()).filter(calculator => calculator.published !== false); },
  async addPatientCalculator(calculatorType, title, description, references = '', published = true) { try { return await rpc('app_add_video', { p_token: token(), p_title: title.trim(), p_theme: published ? CALCULATOR_STORAGE_THEME : CALCULATOR_STORAGE_HIDDEN_THEME, p_description: encodePatientCalculatorContent(description, references), p_url: `https://calculator.local/${calculatorType}`, p_provider: 'youtube', p_embed_url: `calculator://${calculatorType}`, p_thumbnail_url: '' }); } catch { return { success: false, message: 'Não foi possível salvar a calculadora no banco.' }; } },
  async updatePatientCalculator(id, calculatorType, title, description, references = '', published) { try { return await rpc('app_update_video', { p_token: token(), p_id: id, p_title: title.trim(), p_theme: published ? CALCULATOR_STORAGE_THEME : CALCULATOR_STORAGE_HIDDEN_THEME, p_description: encodePatientCalculatorContent(description, references), p_url: `https://calculator.local/${calculatorType}`, p_provider: 'youtube', p_embed_url: `calculator://${calculatorType}`, p_thumbnail_url: '' }); } catch { return { success: false, message: 'Não foi possível atualizar a calculadora no banco.' }; } },
  async deletePatientCalculator(id) { try { return await rpc('app_delete_video', { p_token: token(), p_id: id }); } catch { return { success: false, message: 'Não foi possível excluir a calculadora.' }; } },
