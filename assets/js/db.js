...
function normalizePatientCalculator(calculator) { if (!calculator) return calculator; const source = calculator.embedUrl || calculator.embed_url || ''; const inferredType = /^calculator:\/\/(bmi|waist_height)$/.exec(source)?.[1] || 'bmi'; const isHidden = calculator.theme === CALCULATOR_HIDDEN_THEME; return { ...calculator, calculatorType: calculator.calculatorType || calculator.calculator_type || inferredType, title: calculator.title || 'Calculadora', description: calculator.description || '', published: calculator.published !== false && !isHidden, createdAt: calculator.createdAt || calculator.created_at, updatedAt: calculator.updatedAt || calculator.updated_at }; }
...
export const db = {
  ...
  async addPatientCalculator(calculatorType, title, description, published = true) { try { return await rpc('app_add_video', { p_token: token(), p_title: title.trim(), p_theme: published ? CALCULATOR_THEME : CALCULATOR_HIDDEN_THEME, p_description: (description || '').trim(), p_url: `https://calculator.local/${calculatorType}`, p_provider: 'youtube', p_embed_url: `calculator://${calculatorType}`, p_thumbnail_url: '' }); } catch { return { success: false, message: 'Não foi possível salvar a calculadora no banco.' }; } },
  async updatePatientCalculator(id, calculatorType, title, description, published) { try { return await rpc('app_update_video', { p_token: token(), p_id: id, p_title: title.trim(), p_theme: published ? CALCULATOR_THEME : CALCULATOR_HIDDEN_THEME, p_description: (description || '').trim(), p_url: `https://calculator.local/${calculatorType}`, p_provider: 'youtube', p_embed_url: `calculator://${calculatorType}`, p_thumbnail_url: '' }); } catch { return { success: false, message: 'Não foi possível atualizar a calculadora no banco.' }; } },
