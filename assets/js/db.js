...
const DEFAULT_PATIENT_CALCULATOR = { id: 'builtin-bmi', calculator_type: 'bmi', title: 'Calculadora de IMC', description: 'Calcule seu Índice de Massa Corporal (IMC) de forma simples.', published: true };
...
export const db = {
  ...
  async getPatientCalculators() { const patientFallback = () => currentSession()?.role === 'patient' ? [normalizePatientCalculator(DEFAULT_PATIENT_CALCULATOR)] : []; const local = JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').map(normalizePatientCalculator); let remote = []; try { remote = (await rpc('app_list_videos', { p_token: token() }) || []).filter(video => video.theme === CALCULATOR_THEME || video.theme === CALCULATOR_HIDDEN_THEME).map(normalizePatientCalculator); } catch {} const merged = [...remote]; local.forEach(calculator => { if (!merged.some(item => item.id === calculator.id)) merged.push(calculator); }); return merged.length ? merged : patientFallback(); },
