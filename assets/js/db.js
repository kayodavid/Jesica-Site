...
export const db = {
  ...
  async getPatientCalculators() { try { return (await rpc('app_list_videos', { p_token: token() }) || []).filter(video => video.theme === CALCULATOR_THEME || video.theme === CALCULATOR_HIDDEN_THEME).map(normalizePatientCalculator); } catch { return JSON.parse(localStorage.getItem(DB_KEY_CALCULATORS) || '[]').map(normalizePatientCalculator); } },
